...
BigQuery has a permanent free tier; Snowflake's free option is a time-limited trial. Keith already has professional Snowflake experience (Salesforce/Marketo/LMS star schema work), so BigQuery also demonstrates platform range rather than just repeating what he already knows.

## D002 — One flagship project instead of two structurally distinct projects

Originally scoped as two separate projects (a BigQuery/dbt project and a Databricks/PySpark project) to show platform breadth. Rescoped to one project that covers all three target roles (DA/AE/DE) plus the AI component, since that's the more direct way to demonstrate range without splitting focus. Databricks/PySpark remains an optional secondary project for platform breadth, not required.
...
Airflow is the more commonly requested orchestrator in AE/DE job postings — safer keyword match for a job-search-driven portfolio project. Dagster's tighter native dbt integration was considered and is a legitimate alternative, not chosen here.

## D007 — BI tool: Superset over Metabase

Metabase is easier to stand up; Superset is more complex to operate but shows up more often in DE/AE postings and demonstrates handling a more substantial open-source tool. Chosen for portfolio signal over ease of setup.
...
- **Branching:** trunk-based — `main` always deployable, short-lived feature branches merged via PR (squash merge). Explicitly not GitFlow: GitFlow's develop/release/hotfix branches exist to manage parallel release trains and hotfixing live production separately from in-progress development, neither of which applies to a solo portfolio project with one deployable target. Trunk-based + PRs still shows real review discipline without that overhead.
- **Commits:** Conventional Commits (`feat`, `fix`, `docs`, `chore`, `refactor`, `test`, `ci`) — industry standard, and a clean conventional history is itself part of the portfolio signal.
